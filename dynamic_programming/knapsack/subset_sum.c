/**
  subset sum problem, 0-1 knapsack
  Goal: for a knapsack of # capacity, and items with weights, fill the knapsack to approach max weight
	...
	for (int i = 1; i <= item_count; i++) {
		...
		for (int w = 1; w <= capacity; w++) {
			if (items[i - 1]->weight > w) {
				...
				table[i][w] = max(table[i - 1][w], items[idx]->weight + table[i - 1][w - items[idx]->weight]);
			}
		}
	}
